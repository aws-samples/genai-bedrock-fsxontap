...
You will be using Terraform to provision AWS resources with infrastucture-as-code. All the components with their required configurations as described in the solution overview section.  Follow the instructions below.
...
```
This process can take 30–50 minutes to complete.
...
2. Mount FSx for ONTAP data volume as a network drive.  Use the command below in your cmd line.  Afterwards you should be able ot see the Z: in windows explorer
```
net use Z: \\brsvm.bedrock-01.com\c$\bedrockrag
```
...
4. Upload the [FSx for ONTAP user guide](https://docs.aws.amazon.com/pdfs/fsx/latest/ONTAPGuide/ONTAPGuide.pdf#getting-started) to the shared drive and ensure permissions are set to Everyone
...
#### Start periodic migration of FSx for ONTAP data to AOSS

The Embeddings container will periodically populate an index in the AOSS vector search collection with existing files and folders along with their security access control list (ACL) configurations from the FSx for ONTAP file system.
...
> Troubleshooting: If your Embedding container is not running, you may need to mount FSx for ONTAP or restart the embedding process. 
>
> 1. Navigate to FSx for ONTAP on your AWS console and click on **Storage Virtual Machines** and obtain values for the SVM ID and File System ID parameters. 
...
For the prompt query, ask any general question on the FSx for ONTAP user guide that is available for access to everyone. You will see a response in the chat window as well as the source attribution used by the model for the response.
...
1. Here’s the curl request you can use for invoking API Gateway with *Everyone* access for a query related to the FSx for ONTAP user guide. Note that the value of the *metadata* parameter is set to *NA* to indicate *Everyone* access. 
```
curl -v '<api-invoke-url>/bedrock_rag_retreival' -X POST -H 'content-type: application/json' -d '{"session_id": "1","prompt": "What is an FSx for ONTAP filesystem?", "bedrock_model_id": "anthropic.claude-3-sonnet-20240229-v1:0", "model_kwargs": {"temperature": 1.0, "top_p": 1.0, "top_k": 500}, "metadata": "NA", "memory_window": 10}'
```
...
```
curl -v '<api-invoke-url>/bedrock_rag_retreival' -X POST -H 'content-type: application/json' -d '{"session_id": "1","prompt": "What regions is Bedrock available in?", "bedrock_model_id": "anthropic.claude-3-sonnet-20240229-v1:0", "model_kwargs": {"temperature": 1.0, "top_p": 1.0, "top_k": 500}, "metadata": "S-1-5-21-4037439088-1296877785-2872080499-1112", "memory_window": 10}'
```
